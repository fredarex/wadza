...
import { editCollectionBySlug, updateCollection } from '../../../api/collection'
...
import { ICreatorFee, ICollectionData, ICollectionErrorMsg, ICollectionTheme, IPaymentToken, IMockCategory, ICommonProps } from '../../../types/types'
...
const styles = {
  ...
  button: 'flex flex-row justify-center items-center max-w-[243px] w-full h-[58px] font-poppins-600 text-base text-white leading-[98.3%] rounded-md mt-[72px] hover:bg-purple-light ease-in duration-100',
}

const CollectionEdit = (props: IProps) => {
  const { slug } = props
  const intl = useIntl()
  ...
  useEffect(() => {
    const getData = async () => {
      if (slug) {
        const getCollectionResult = await editCollectionBySlug(slug)
  
        if (!getCollectionResult?.data?.error && getCollectionResult?.data?.data) {
          let _collectionData = getCollectionResult?.data?.data
  
          // category
          const _category = _collectionData?.category as string[] || []
          if (_category.length > 0) {
            const _categories: IMockCategory[] = _category.map((cat: any) => {
              const label = CategoryData.filter((category: IMockCategory) => category.value === cat)
              return {
                label: label[0].label,
                value: cat,
              }
            ...
            const _cats = categories.filter((category: IMockCategory) => {
              return !_category.includes(category.value)
            })
            setCategories(_cats)
          }
  
          // payment tokens
          const _paymentTokens = _collectionData?.paymentTokens as any[] || []
          if (_paymentTokens.length > 0) {
            const paymentTokens: IPaymentToken[] = _paymentTokens.map((pT: any) => {
              return {
                ...pT,
                icon: pT.symbol === 'ETH' ? ETHIcon : pT.symbol === 'WETH' ? WETHIcon : null,
                removable: ['ETH', 'WETH'].includes(pT.symbol) ? false : true
              }
            })
            ...
            const _options = options.filter((option: IPaymentToken) => {
              const addedTokens = _paymentTokens.filter((pT: any) => pT.symbol === option.symbol)
              return addedTokens.length === 0
            })
            setOptions(_options)
          } else {
            _collectionData = {
  ...
  const onClickUpdate = useCallback(
    async () => {
      try {
        ...
        if (dataValidation()) {
          const _paymentTokens = collectionData?.paymentTokens.map((paymentToken: IPaymentToken) => {
            return {
          ...
          const _categories = collectionData?.category.map((category: IMockCategory) => {
          ...
          formData.append('explicit', collectionData?.explicit || false)
          formData.append('rarity', collectionData?.rarity || false)
          ...
          updateCollection(formData, collectionData?._id!)
            .then((data) => {
              if (!data?.data?.error && data?.data?.data) {
                let _collectionData = data?.data?.data
              ...
              toast.update(collectionCreation, { render: 'Collection was updated successfully', type: 'success', isLoading: false, autoClose: 3000 })
              setLoading(false)
            })
            .catch((error) => {
              toast.update(collectionCreation, { render: 'Collection was not updated', type: 'error', isLoading: false, autoClose: 3000 })
              setLoading(false)
              console.error(error)
            })
        }
      ...
      // eslint-disable-next-line
    }, [dataValidation, collectionData, user]
  )
      ...
      <div className='flex flex-col max-w-[1250px] w-full'>
        <div className='flex flex-row justify-center items-center max-w-[434px] w-full bg-[#DFDBE4] px-13 py-8 rounded-t-[15px]'>
          <h1 className='font-poppins-700 text-3xl text-black-lighter'>
            <FormattedMessage id='page.collection_edit.title' />
          </h1>
        </div>
        <div className='w-full bg-purple-lightest px-[52px] pt-[38px] pb-[70px] rounded-[0px_15px_15px_15px]'>
            ...
            <div className='flex flex-col'>
              ...
              <section>
                ...
                <div className={styles.logoImage} {...getLogoImageRootProps()} onClick={logoImageOpen}>
                  ...
                  {collectionData?.logoImage && <div className='absolute w-full h-full box-border'>
                    {typeof collectionData?.logoImage === 'string'? <picture><img src={collectionData?.logoImage} alt='collection logo image' className='min-w-[100%] max-w-[100%] min-h-[100%] max-h-[100%] rounded-full object-cover' /></picture> : <picture><img src={URL.createObjectURL(collectionData?.logoImage)} alt='collection logo image' className='min-w-[100%] max-w-[100%] min-h-[100%] max-h-[100%] rounded-full object-cover' /></picture>}
                  </div>}
              ...
              <section className='mt-[62px]'>
                ...
                <div className={styles.featuredImage} {...getFeaturedImageRootProps()} onClick={featuredImageOpen}>
                  ...
                  {collectionData?.featuredImage && <div className='absolute w-full h-full box-border'>
                    {typeof collectionData?.featuredImage === 'string'? <picture><img src={collectionData?.featuredImage} alt='collection featured image' className='min-w-[100%] max-w-[100%] min-h-[100%] max-h-[100%] rounded-[14px] object-cover' /></picture> : <picture><img src={URL.createObjectURL(collectionData?.featuredImage)} alt='collection featured image' className='min-w-[100%] max-w-[100%] min-h-[100%] max-h-[100%] rounded-[14px] object-cover' /></picture>}
              ...
              <section className='mt-[68px]'>
                ...
                <div className={styles.bannerImage} {...getBannerImageRootProps()} onClick={bannerImageOpen}>
                  ...
                  {typeof collectionData?.bannerImage === 'string'? <picture><img src={collectionData?.bannerImage} alt='collection banner image' className='min-w-[100%] max-w-[100%] min-h-[100%] max-h-[100%] rounded-[14px] object-cover' /></picture> : <picture><img src={URL.createObjectURL(collectionData?.bannerImage)} alt='collection banner image' className='min-w-[100%] max-w-[100%] min-h-[100%] max-h-[100%] rounded-[14px] object-cover' /></picture>}
                    <div className='absolute top-3 right-3 z-20' onClick={(e) => {
                      e.stopPropagation()
              ...
              <section className='mt-[66px]'>
                ...
                <Switch
                  checked={collectionData?.explicit || false}
              ...
              <section className='mt-[66px]'>
                ...
                <Switch
                  checked={collectionData?.rarity || false}
          ...
          {/* create button */}
          <button onClick={() => onClickUpdate()} disabled={loading} className={`${styles.button} ${loading ? 'bg-purple-light' : 'bg-purple'}`}>
