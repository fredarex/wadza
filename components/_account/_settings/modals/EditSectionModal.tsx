...
import { ArrowDownSvgIcon, ArrowLeftSvgIcon, CloseSvgIcon } from '../../../icons'
...
interface IProps extends IModal {
  ...
  update: (data: any, featuredItemId: string) => void
  updateLoading: boolean
  selectedItem: any
}

const EditSectionModal = (props: IProps) => {
  const { isOpen, title, close, collectedListings, collectedLoading, update, updateLoading, selectedItem } = props
  const intl = useIntl()
  ...
  useEffect(() => {
    setSection('init')
    setSelectedListings(selectedItem?.listings)
    setItemDetail({
            ...
            >
              ...
              <div className='flex flex-col w-full bg-white rounded-[15px]'>
                ...
                <div className='flex items-start justify-between pt-[23px] pb-5 px-[26px] border-b-2 border-solid border-[rgba(139,110,174,0.13)] rounded-t'>
                  <span className='w-[23px]'>{section === 'init'? '' : <span className='cursor-pointer' onClick={() => setSection('init')}><ArrowLeftSvgIcon color='#3C1361' /></span>}</span>
                ...
                {section === 'done' && <div className='px-[34px] py-[26px]'>
                  ...
                  <button onClick={() => updateFeaturedItem()} disabled={updateLoading} className={`flex flex-row justify-center items-center w-full h-[58px] ${updateLoading ? 'bg-purple-light' : 'bg-purple'} rounded-[14px] mt-[55px] hover:bg-purple-light hover:scale-[1.02] hover:shadow-sm transition ease-in duration-150 active:scale-[1]`}>
                    {updateLoading ? <Lottie animationData={Loading} style={{ width: 25, height: 40, }} /> : <h4 className='font-poppins-600 text-base text-white leading-[98.3%]'>
