...
const WatchlistCollectionCard = (props: IProps) => {
  ...
  return (
    <div className='flex flex-row justify-end items-center w-full h-[97px] px-[38px] bg-[#DDD7E5] mt-[10px] rounded-lg relative cursor-pointer'>
      <div className='flex items-center w-full py-[15px] my-[10px]'>
        <picture><img
          src={collection?.image ? collection.image : process.env.NEXT_PUBLIC_IMAGE_PLACEHOLDER}
          alt='collection image'
          className='max-w-[55px] sm:max-w-[71px] w-full h-[60px] sm:h-[67px] mr-[10px] sm:mr-[22px] object-cover rounded'
          onClick={() => handleCollectionClick(collection?.slug!)}
        /></picture>
        <div className='flex-col w-[27%]' onClick={() => handleCollectionClick(collection?.slug!)}>
        ...
        <div className='flex w-[17.4%] items-center' onClick={() => handleCollectionClick(collection?.slug!)}>
        ...
        <div className='flex w-[17.6%] items-center' onClick={() => handleCollectionClick(collection?.slug!)}>
          <h2 className={`font-poppins-700 text-black text-base leading-[98.3%]`}>
            {`${Number(collection?.floorPrice) === 0 ? '-' : `${Number(collection?.floorPrice).toFixed(3)} ETH`}`}
          </h2>
        </div>
        <div className='flex w-[7%] items-center' onClick={() => handleCollectionClick(collection?.slug!)}>
          <h2 className={`font-poppins-700 text-black text-base leading-[98.3%]`}>
            {`${Number(collection?.sales) === 0 ? '-' : `${Number(collection?.sales)}`}`}
          </h2>
        </div>
        <div className='flex items-center' onClick={() => handleWatchlist()}>
...
export default WatchlistCollectionCard
