...
import { getTopRankingsCollections } from '../../api/collection'
...
const TopRankings = (props: IProps) => {
  const { category, timeRange, className } = props
  ...
  useEffect(() => {
    const getCollections = async () => {
      setLoading(true)
      const collectionResults = await getTopRankingsCollections({
        category,
        timeRange,
      })
