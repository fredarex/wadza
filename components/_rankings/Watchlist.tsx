...
import { getWatchlist } from '../../api/collection'
import WatchlistCollectionCard from './cards/WatchlistCollectionCard'
...
const Watchlist = (props: IProps) => {
  const { timeRange, className } = props
  ...
  useEffect(() => {
    const getCollections = async () => {
      setLoading(true)
      const collectionResults = await getWatchlist({
        timeRange,
      })
  ...
  const sortByVolume = useCallback(() => {
    ...
    const _sortedCollections = staticCollections.current.sort((prev, next) => {
      if (volumeSort) {
        return Number(next.volume) - Number(prev.volume)
      } else {
        return Number(prev.volume) - Number(next.volume)
      }
    })
  ...
  return (
    <div className='flex flex-col max-w-[1250px] w-full mt-2'>
      ...
      {/* body */}
      <div className='w-full bg-[#E6E2EC] p-[0_10px_48px_10px] rounded-b-[15px]'>
        {loading? animations.map((a: any, index: number) => (
